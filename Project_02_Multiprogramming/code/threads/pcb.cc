...
// Tiến trình chuyển sang trạng thái block và ngừng lại, chờ JoinRelease để thực hiện tiếp.
void PCB::JoinWait()
{
    joinsem->P();
}
...
// Tiến trình chuyển sang trạng thái block và ngừng lại, chờ ExitReleaseđể thực hiện tiếp.
void PCB::ExitWait()
{
    exitsem->P();
}
...
{
    multex->P(); // Call mutex->P() to help avoid loading 2 processes at the same time
    ...
    thread->Fork((VoidFunctionPtr)&StartProcess, (void *)arg); // Fork a thread to run StartProcess
    multex->V();
    return pid;
}
