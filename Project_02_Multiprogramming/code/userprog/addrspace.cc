...
bool AddrSpace::Load(char *fileName)
{
    ...
    for (unsigned int i = 0; i < numPages; ++i)
    {
        ...
        // Tìm frame trống trong Physical mem
        pageTable[i].physicalPage = kernel->gPhysPageBitMap->FindAndSet();
        // Set default like monoprogramming
        pageTable[i].valid = TRUE;
        pageTable[i].use = FALSE;
        pageTable[i].dirty = FALSE;
        pageTable[i].readOnly = FALSE;
        memset(&kernel->machine->mainMemory[pageTable[i].physicalPage * PageSize], 0, PageSize);
    }
    // allocate done
    kernel->addrLock->V();
    ...
    if (noffH.initData.size > 0)
    {
        ...
        if (lastCodePageSize != PageSize && numCodePage > 0)
        { // if remain slot
            executable->ReadAt(
